...
class AuthManager {
  ...
  async initialize() {
    if (this.mode === "stdio" || this.mode === "http") {
      ...
      if (!this.apiKey) {
        throw new AuthenticationError("Tally API key not found in environment variables");
      }
    }
  ...
  getApiKey() {
    ...
    if (!this.apiKey) {
      throw new AuthenticationError(this.mode === "stdio" || this.mode === "http" ? "API key not initialized" : "API key not set");
    }
    return this.apiKey;
  }
  setApiKey(key) {
...
class QueryCache {
  ...
  constructor(defaultMaxAge = 300000) {
...
class TallyGraphQLClient {
  ...
  constructor(authManager, options = {}) {
    if (!authManager.hasApiKey()) {
      throw new AuthenticationError("AuthManager must be initialized before creating GraphQL client");
    }
    ...
    if (this.options.enableRateLimit) {
      this.rateLimiter = new RateLimiter(this.options.maxRequestsPerMinute, 60000);
    }
  }
  getEndpoint() {
  ...
  validateVariables(query, variables) {
    try {
      const document = import_graphql.parse(query);
  ...
  async executeWithRetry(query, variables, attempt = 0) {
    try {
      if (this.rateLimiter && !this.rateLimiter.canMakeRequest()) {
        const retryAfter = this.rateLimiter.getRetryAfter();
        throw new RateLimitError(`Rate limit exceeded. Please retry after ${retryAfter} seconds.`, retryAfter);
      }
      if (this.rateLimiter) {
      ...
      if (error instanceof GraphQLNetworkError && error.status === 429) {
        ...
        throw new RateLimitError(`API rate limit exceeded. Please retry after ${retryAfter} seconds.`, retryAfter);
      }
      if (attempt < this.options.retryAttempts && error instanceof GraphQLNetworkError && error.status !== 429) {
  ...
  async executeQuery(query, variables) {
    ...
    if (!apiKey) {
      throw new Error("Tally API key not configured. Please set TALLY_API_KEY environment variable.");
    }
    ...
    if (!response.ok) {
      throw new GraphQLNetworkError(`HTTP ${response.status}: ${response.statusText}`, response.status);
    }
    const result = await response.json();
    if (result.errors) {
      const errorMessages = result.errors.map((err) => err.message).join(", ");
      throw new GraphQLClientError(`GraphQL errors: ${errorMessages}`, result.errors);
    }
    return result.data;
  }
...
// src/organization-tools.ts
import { z } from "zod";
var ListOrganizationsInputSchema = z.object({
...
var GetOrganizationInputSchema = z.object({
  ...
  message: "Either organizationId or organizationSlug must be provided, but not both"
});
var GetOrganizationsWithActiveProposalsInputSchema = z.object({
  page: z.number().int().positive().optional(),
  pageSize: z.number().int().positive().max(100).optional(),
  minActiveProposals: z.number().int().positive().optional(),
  chainId: z.string().optional()
});
function validateListOrganizationsInput(input) {
  return ListOrganizationsInputSchema.safeParse(input);
}
function validateGetOrganizationInput(input) {
  return GetOrganizationInputSchema.safeParse(input);
}
function validateGetOrganizationsWithActiveProposalsInput(input) {
  return GetOrganizationsWithActiveProposalsInputSchema.safeParse(input);
}
async function listOrganizations(client, input) {
  const validation = validateListOrganizationsInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for listOrganizations", validation.error.errors);
  }
  const {
    page = 1,
    pageSize = 20,
  ...
  const transformedOrganizations = result.organizations.nodes.map((org) => ({
    id: org.id,
    name: org.name,
    slug: org.slug,
...
async function getOrganization(client, input) {
  const validation = validateGetOrganizationInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getOrganization", validation.error.errors);
  }
...
async function getOrganizationsWithActiveProposals(client, input) {
  const validation = validateGetOrganizationsWithActiveProposalsInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getOrganizationsWithActiveProposals", validation.error.errors);
  }
  const {
    pageSize = 20,
    chainId
  ...
  const result = await client.query(query, variables);
  const transformedOrgs = result.organizations.nodes.map((org) => ({
    id: org.id,
    name: org.name,
    slug: org.slug,
    chainId: org.chainIds[0] || "",
...
var ListProposalsInputSchema = z2.object({
  organizationId: z2.string().trim().min(1),
  page: z2.number().int().positive().optional(),
  pageSize: z2.number().int().positive().max(100).optional(),
  governorId: z2.string().optional(),
  proposer: z2.string().optional(),
  sortOrder: z2.enum(["asc", "desc"]).optional()
});
var GetProposalInputSchema = z2.object({
  organizationId: z2.string().trim().min(1).optional(),
  organizationSlug: z2.string().trim().min(1).optional(),
  proposalId: z2.string().trim().min(1)
}).refine((data) => {
  return true;
}, {
  message: "Organization context is recommended but not required"
});
var GetActiveProposalsInputSchema = z2.object({
  page: z2.number().int().positive().optional(),
  pageSize: z2.number().int().positive().max(100).optional(),
  chainId: z2.string().optional(),
  organizationId: z2.string().optional()
});
function validateListProposalsInput(input) {
  return ListProposalsInputSchema.safeParse(input);
}
function validateGetProposalInput(input) {
  return GetProposalInputSchema.safeParse(input);
}
function validateGetActiveProposalsInput(input) {
  return GetActiveProposalsInputSchema.safeParse(input);
}
async function listProposals(client, input) {
  const validation = validateListProposalsInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for listProposals", validation.error.errors);
  }
  const {
    organizationId,
    page = 1,
  ...
  const typedPageSize = pageSize;
  const includeSort = sortOrder === "asc";
  ...
  const result = await client.query(query, variables);
  const proposals = result.proposals.nodes.map((proposal) => ({
    id: proposal.id,
    ...
    votingStats: {
      quorum: "0",
      yesVotes: proposal.voteStats?.find((v) => v.type === "for")?.votesCount || "0",
      noVotes: proposal.voteStats?.find((v) => v.type === "against")?.votesCount || "0",
      abstainVotes: proposal.voteStats?.find((v) => v.type === "abstain")?.votesCount || "0"
    },
    organization: {
      id: "",
...
async function getProposal(client, input) {
  const validation = validateGetProposalInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getProposal", validation.error.errors);
  }
  ...
  return {
    ...
    votingStats: {
      ...
      noVotes: proposal.voteStats?.find((v) => v.type === "against")?.votesCount || "0",
      abstainVotes: proposal.voteStats?.find((v) => v.type === "abstain")?.votesCount || "0",
      totalVotes: proposal.voteStats?.reduce((sum, v) => sum + parseInt(v.votesCount || "0"), 0).toString() || "0"
    },
    tokenInfo: {
      symbol: tokenSymbol,
      name: tokenInfo?.name || "",
    ...
    actions: proposal.executableCalls?.map((call, index) => ({
      id: index.toString(),
      target: call.target,
      value: call.value,
    ...
    executableCalls: proposal.executableCalls?.map((call, index) => ({
      id: index.toString(),
      target: call.target,
      value: call.value,
      signature: call.signature,
...
async function getActiveProposals(client, input) {
  const validation = validateGetActiveProposalsInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getActiveProposals", validation.error.errors);
  }
  const { page = 1, pageSize = 20, chainId, organizationId } = validation.data;
  const typedPage = page;
  const typedPageSize = pageSize;
  if (organizationId) {
    const proposalsQuery2 = `
    ...
    `;
    const result = await client.query(proposalsQuery2, {
      organizationId,
      pageSize: 50
    });
    const votableProposals = result.proposals.nodes.filter((proposal) => proposal.status === "active" || proposal.status === "extended").slice(0, typedPageSize).map((proposal) => ({
      id: proposal.id,
      title: proposal.metadata.title,
      description: proposal.metadata.description,
      status: proposal.status,
  ...
  const activeOrgs = orgsResult.organizations.nodes.filter((org) => org.hasActiveProposals === true);
  ...
  for (const org of activeOrgs) {
    try {
      ...
      if (result.proposals?.nodes) {
        const votableProposals = result.proposals.nodes.filter((proposal) => proposal.status === "active" || proposal.status === "extended").map((proposal) => ({
          id: proposal.id,
          title: proposal.metadata.title,
          description: proposal.metadata.description,
          status: proposal.status,
...
function analyzeTimelockOperations(executableCalls) {
  const operations = [];
  ...
  for (const call of executableCalls) {
    ...
    if (signature.includes("transfer(")) {
      ...
      if (call.decodedCalldata?.parameters) {
        ...
        recipient = params.find((p) => p.name === "to" || p.name === "recipient")?.value;
        amount = params.find((p) => p.name === "amount" || p.name === "value")?.value;
      }
      ...
      if (call.decodedCalldata?.parameters) {
        const params = call.decodedCalldata.parameters;
        recipient = params.find((p) => p.name === "to")?.value;
        amount = params.find((p) => p.name === "amount" || p.name === "value")?.value;
      }
      tokenAddress = call.target;
    } else if (ethValue > 0n) {
    ...
    if (ethValue > BigInt("1000000000000000000") || operationType === "erc20transfer" || operationType === "custom") {
...
var GetUserDAOsInputSchema = z3.object({
  address: z3.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address format"),
  pageSize: z3.number().int().positive().max(100).optional().default(20)
});
var GetDAOParticipantsInputSchema = z3.object({
  organizationId: z3.string().trim().min(1),
  pageSize: z3.number().int().positive().max(100).optional().default(20),
  page: z3.number().int().positive().optional().default(1)
});
var GetUserDetailsInputSchema = z3.object({
  address: z3.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address format")
});
var GetDelegatesInputSchema = z3.object({
  organizationId: z3.string().trim().min(1),
  pageSize: z3.number().int().positive().max(100).optional().default(20),
  sortBy: z3.enum(["id", "votes", "delegators", "isPrioritized"]).optional().default("votes"),
  sortOrder: z3.enum(["asc", "desc"]).optional().default("desc")
});
var GetUserProfileInputSchema = z3.object({
  address: z3.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address format"),
  pageSize: z3.number().int().positive().max(100).optional().default(20)
});
var GetDelegateStatementInputSchema = z3.object({
  address: z3.string().regex(/^0x[a-fA-F0-9]{40}$/, "Invalid Ethereum address format"),
  organizationId: z3.string().trim().min(1)
});
function validateGetDAOParticipantsInput(input) {
  return GetDAOParticipantsInputSchema.safeParse(input);
}
function validateGetDelegatesInput(input) {
  return GetDelegatesInputSchema.safeParse(input);
}
function validateGetUserProfileInput(input) {
  return GetUserProfileInputSchema.safeParse(input);
}
function validateGetDelegateStatementInput(input) {
  return GetDelegateStatementInputSchema.safeParse(input);
}
async function getDAOParticipants(client, input) {
  const validation = validateGetDAOParticipantsInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getDAOParticipants", validation.error.errors);
  }
  ...
  try {
    ...
    const items = result.delegates.nodes.map((delegate) => ({
      id: delegate.id,
...
async function getDelegates(client, input) {
  const validation = validateGetDelegatesInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getDelegates", validation.error.errors);
  }
  ...
  try {
    ...
    const items = result.delegates.nodes.map((delegate) => ({
      ...
      conversionNote: delegate.token?.decimals ? `To convert to human-readable amount: divide ${delegate.votesCount} by 10^${delegate.token.decimals} (Ethereum-style decimal places)` : `To convert to human-readable amount: divide ${delegate.votesCount} by 10^18 (default Ethereum token decimals)`
    }));
    return {
      items,
      totalCount: result.delegates.pageInfo?.count || 0,
...
async function getUserProfile(client, input) {
  const validation = validateGetUserProfileInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getUserProfile", validation.error.errors);
  }
  ...
  try {
    ...
    const daoParticipations = (result.delegatees?.nodes || []).map((delegation) => ({
      id: delegation.id,
      ...
      conversionNote: delegation.token?.decimals ? `To convert to human-readable amount: divide ${delegation.votes} by 10^${delegation.token.decimals} (Ethereum-style decimal places)` : `To convert to human-readable amount: divide ${delegation.votes} by 10^18 (default Ethereum token decimals)`
    }));
    const delegateParticipations = (result.delegates?.nodes || []).map((delegate) => ({
      id: delegate.id,
      organization: delegate.organization,
      statement: delegate.statement && (delegate.statement.statement || delegate.statement.statementSummary || delegate.statement.isSeekingDelegation !== undefined) ? delegate.statement : undefined,
      votesCount: delegate.votesCount,
      delegatorsCount: delegate.delegatorsCount,
      tokenInfo: delegate.token ? {
        symbol: delegate.token.symbol,
...
async function getDelegateStatement(client, input) {
  const validation = validateGetDelegateStatementInput(input);
  if (!validation.success) {
    throw new ValidationError("Invalid input for getDelegateStatement", validation.error.errors);
  }
...
// src/resources/popular-daos.ts
async function getPopularDAOsData(graphqlClient) {
...
async function loadAllChains(graphqlClient) {
  const query = `
    query GetAllChains {
      chains {
        id
...
async function loadPopularDAOs(graphqlClient, productionChains) {
  const chainsToQuery = productionChains.slice(0, 8);
  const queries = chainsToQuery.map((chain) => fetchDAOsForChain(graphqlClient, chain));
  ...
  const sortedDAOs = allDAOs.sort((a, b) => b.delegateCount - a.delegateCount).slice(0, 20);
  return sortedDAOs;
}
async function fetchDAOsForChain(graphqlClient, chain) {
  const query = `
...
function generateChainsLookup(chains) {
  const chainsById = {};
...
// src/resources/organization-overview.ts
async function getOrganizationOverview(graphqlClient, organizationId) {
  try {
...
function generateOrganizationMarkdown(org) {
  const sections = [];
  ...
  const links = [];
  if (org.website)
    links.push(`[Website](${org.website})`);
  if (org.twitter)
    links.push(`[Twitter](${org.twitter})`);
  if (org.github)
    links.push(`[GitHub](${org.github})`);
  if (links.length > 0) {
    sections.push("## \uD83D\uDD17 Links");
    sections.push("");
...
// src/resources/proposal-overview.ts
async function getProposalOverview(graphqlClient, organizationId, proposalId) {
  try {
...
function generateProposalMarkdown(proposal, organizationId, proposalId) {
  const sections = [];
  ...
  if (proposal.votingStats) {
    ...
    if (stats.for !== undefined)
      sections.push(`- **For:** ${formatVotes(stats.for)}`);
    if (stats.against !== undefined)
      sections.push(`- **Against:** ${formatVotes(stats.against)}`);
    if (stats.abstain !== undefined)
      sections.push(`- **Abstain:** ${formatVotes(stats.abstain)}`);
...
// src/resources/user-overview.ts
async function getUserOverview(graphqlClient, address) {
  try {
...
function generateUserMarkdown(userProfile, address) {
  const sections = [];
  ...
  if (userProfile.participations && userProfile.participations.length > 0) {
    ...
    sections.push("");
    userProfile.participations.forEach((participation) => {
  ...
  if (userProfile.participations && userProfile.participations.length > 0) {
    const totalProposals = userProfile.participations.reduce((sum, p) => sum + (p.proposalsCreated || 0), 0);
    const totalVotes = userProfile.participations.reduce((sum, p) => sum + (p.votesCast || 0), 0);
    const delegateCount = userProfile.participations.filter((p) => p.delegate).length;
...
function formatVotingPower(votingPower) {
...
// src/resources/trending-proposals.ts
async function getTrendingProposalsOverview(graphqlClient) {
  try {
...
function generateTrendingProposalsMarkdown(data) {
  const sections = [];
  ...
  if (proposals.length === 0) {
    ...
    sections.push("");
    Object.entries(proposalsByOrg).forEach(([orgName, orgProposals]) => {
      sections.push(`### ${orgName} (${orgProposals.length} proposal${orgProposals.length > 1 ? "s" : ""})`);
      sections.push("");
      orgProposals.forEach((proposal) => {
        const statusEmoji = getStatusEmoji2(proposal.status);
        ...
        sections.push("");
        if (proposal.endTime) {
          sections.push(`- **Voting Ends:** ${formatDate2(proposal.endTime)}`);
        }
        if (proposal.votingStats) {
          const stats = proposal.votingStats;
          if (stats.total !== undefined && stats.total > 0) {
            sections.push(`- **Total Votes:** ${formatVotes2(stats.total)}`);
          }
        }
        if (proposal.proposer?.address) {
...
function groupProposalsByOrganization(proposals) {
  const grouped = {};
...
// src/prompts/governance-prompts.ts
import { z as z4 } from "zod";
var governancePrompts = {
  "analyze-dao-governance": {
    schema: {
      daoName: z4.string().describe('The name of the DAO to analyze (e.g., "Wormhole", "Arbitrum", "Uniswap")'),
      includeComparison: z4.string().optional().describe("Whether to compare with other similar DAOs (true/false)")
    },
    handler: ({ daoName, includeComparison }) => {
...
Provide actionable insights about "${daoName}"'s governance strengths and areas for improvement.`
  ...
  "compare-dao-governance": {
    schema: {
      dao1Name: z4.string().describe('First DAO name (e.g., "Arbitrum", "Wormhole")'),
      dao2Name: z4.string().describe('Second DAO name (e.g., "Uniswap", "Compound")'),
      aspect: z4.string().describe("Specific aspect to focus the comparison on (overall, delegates, proposals, activity)")
    },
    handler: ({ dao1Name, dao2Name, aspect }) => {
      ...
      const instruction = aspectInstructions[aspect] || aspectInstructions.overall;
      return {
        messages: [{
          role: "user",
          content: {
...
Present your findings in a clear, structured format with supporting data.`
  ...
  "analyze-delegate-profile": {
    schema: {
      address: z4.string().describe("Ethereum address of the delegate"),
      daoName: z4.string().optional().describe('Specific DAO name to focus on (optional, e.g., "Arbitrum")')
    },
    handler: ({ address, daoName }) => ({
      messages: [{
        role: "user",
        content: {
          type: "text",
...
**Output**: Provide a comprehensive delegate profile that would help token holders make informed delegation decisions${daoName ? ` in "${daoName}"` : " across the ecosystem"}.`
  ...
  "discover-governance-trends": {
    schema: {
      timeframe: z4.string().describe("Time focus for trend analysis (current, recent, emerging)"),
      category: z4.string().optional().describe("Category of DAOs to focus on (all, defi, infrastructure, social)")
    },
    handler: ({ timeframe, category }) => {
      ...
      const instruction = timeframeInstructions[timeframe] || timeframeInstructions.current;
      return {
        messages: [{
          role: "user",
          content: {
...
**Deliverable**: A trend report highlighting the most significant governance developments, with specific examples and data to support your findings.`
  ...
  "find-dao-to-join": {
    schema: {
      interests: z4.string().describe('User interests or focus areas (comma-separated, e.g., "DeFi,gaming,social impact")'),
      participationLevel: z4.string().describe("Desired level of participation (observer, voter, delegate, contributor)"),
      experience: z4.string().describe("Governance experience level (beginner, intermediate, expert)")
    },
    handler: ({ interests, participationLevel, experience }) => {
...
**Recommendations**: Provide 3-5 specific DAO recommendations with rationale for each, including how to get started and what to expect.`
  ...
  "analyze-proposal": {
    schema: {
      daoName: z4.string().describe('Name of the DAO (e.g., "Wormhole", "Arbitrum")'),
      proposalTitle: z4.string().describe("Title or description of the proposal to analyze")
    },
    handler: ({ daoName, proposalTitle }) => ({
      messages: [{
        role: "user",
        content: {
          type: "text",
...
var servers = {};
var monitors = {};
var ENABLE_SSE = process.env.ENABLE_SSE === "true";
...
async function handlePostRequest(req, res) {
  ...
  if (ENABLE_SSE) {
    if (sessionId && transports[sessionId]) {
      ...
      monitor = null;
      setupAllTools(sessionServer, graphqlClient);
      setupAllResources(sessionServer, graphqlClient);
      setupAllPrompts(sessionServer);
    ...
    await authManager.initialize();
    const graphqlClient = new TallyGraphQLClient(authManager);
    setupAllTools(server, graphqlClient);
    setupAllResources(server, graphqlClient);
    setupAllPrompts(server);
    transport = new StreamableHTTPServerTransport({
...
function setupAllTools(server, graphqlClient) {
  server.tool("get_server_info", "Get information about the MCP Tally API server", {}, async () => {
    return {
      content: [{
        type: "text",
        text: JSON.stringify({
          ...
          transport: ENABLE_SSE ? "http-with-sse" : "http-stateless",
          ...
          resourceCount: 7,
          promptCount: 6,
          tally_api_configured: !!process.env.TALLY_API_KEY
        }, null, 2)
      }]
    };
  });
  server.tool("list_organizations", "List organizations with pagination, filtering, and sorting options", {
    page: z5.number().optional().describe("Page number (default: 1)"),
    pageSize: z5.number().optional().describe("Number of organizations per page (max: 100, default: 20)"),
    chainId: z5.string().optional().describe('Filter by blockchain chain ID (e.g., "eip155:1" for Ethereum mainnet)'),
    hasLogo: z5.boolean().optional().describe("Filter by whether organization has a logo"),
    sortBy: z5.string().optional().describe("Sort field: id (date), name, explore (by activity), popular (default: name)"),
    sortOrder: z5.string().optional().describe("Sort order: asc or desc (default: asc)")
  }, async (args) => {
    try {
  ...
  server.tool("get_organization", "Get detailed information about a specific organization by ID or slug", {
    organizationId: z5.string().optional().describe("Organization ID (use either this or organizationSlug)"),
    organizationSlug: z5.string().optional().describe("Organization slug (use either this or organizationId)")
  }, async ({ organizationId, organizationSlug }) => {
    try {
  ...
  server.tool("get_organizations_with_active_proposals", "Get organizations that have active proposals with filtering options", {
    minActiveProposals: z5.number().optional().describe("Minimum number of active proposals (default: 1)"),
    chainId: z5.string().optional().describe("Filter by chain ID"),
    page: z5.number().optional().describe("Page number (default: 1)"),
    pageSize: z5.number().optional().describe("Number of organizations per page (max: 100, default: 20)")
  }, async ({ minActiveProposals, chainId, page, pageSize }) => {
    try {
      const result = await getOrganizationsWithActiveProposals(graphqlClient, {
        page,
        pageSize,
        minActiveProposals,
        chainId
  ...
  server.tool("list_proposals", "List proposals for a specific organization with pagination, filtering, and sorting", {
    organizationId: z5.string().describe("Organization ID (required)"),
    page: z5.number().optional().describe("Page number (default: 1)"),
    pageSize: z5.number().optional().describe("Number of proposals per page (max: 100, default: 20)"),
    governorId: z5.string().optional().describe("Filter by governor contract ID"),
    proposer: z5.string().optional().describe("Filter by proposer address"),
    sortOrder: z5.string().optional().describe("Sort order: asc or desc (default: desc)")
  }, async (args) => {
    try {
  ...
  server.tool("get_proposal", "Get detailed information about a specific proposal", {
    organizationId: z5.string().optional().describe("Organization ID (use either this or organizationSlug)"),
    organizationSlug: z5.string().optional().describe("Organization slug (use either this or organizationId)"),
    proposalId: z5.string().describe("Proposal ID (required)")
  }, async ({ organizationId, organizationSlug, proposalId }) => {
    try {
  ...
  server.tool("get_user_profile", "Get comprehensive user profile including user details and DAO participations", {
    address: z5.string().describe("Ethereum address of the user (required)"),
    pageSize: z5.number().optional().describe("Number of DAO participations per page (max: 100, default: 20)")
  }, async ({ address, pageSize }) => {
    try {
  ...
  server.tool("get_delegate_statement", "Get delegate statement for a specific user and organization", {
    address: z5.string().describe("Ethereum address of the delegate (required)"),
    organizationId: z5.string().describe("Organization ID (required)")
  ...
  server.tool("get_dao_participants", "Get participants of a specific DAO with pagination, filtering, and sorting", {
    organizationId: z5.string().describe("Organization ID (required)"),
    pageSize: z5.number().optional().describe("Number of participants per page (max: 100, default: 20)")
  }, async ({ organizationId, pageSize }) => {
    try {
  ...
  server.tool("get_delegates", "Get enhanced delegate information for a specific organization including voting power, account details, statements, and organization info", {
    organizationId: z5.string().describe("Organization ID (required)"),
    pageSize: z5.number().optional().describe("Number of delegates per page (max: 100, default: 20)"),
    sortBy: z5.string().optional().describe("Sort field: id, votes, delegators, isPrioritized (default: votes)"),
    sortOrder: z5.string().optional().describe("Sort order: asc or desc (default: desc)")
  }, async ({ organizationId, pageSize, sortBy, sortOrder }) => {
    try {
  ...
  server.tool("execute_graphql_query", "Execute an arbitrary GraphQL query against the Tally API", {
    query: z5.string().describe("GraphQL query string"),
    variables: z5.record(z5.any()).optional().describe("Optional variables for the GraphQL query")
...
function setupAllResources(server, graphqlClient) {
  server.resource("server-info", "tally://server/info", { mimeType: "application/json" }, async () => {
    return {
      contents: [{
  ...
  server.resource("popular-daos", "tally://popular-daos", { mimeType: "application/json" }, async () => {
  ...
  server.resource("organization-overview", new ResourceTemplate("tally://org/{organizationId}", { list: undefined }), async (uri, params) => {
  ...
  server.resource("proposal-overview", new ResourceTemplate("tally://org/{organizationId}/proposal/{proposalId}", { list: undefined }), async (uri, params) => {
    const organizationId = Array.isArray(params.organizationId) ? params.organizationId[0] : params.organizationId;
  ...
  server.resource("user-overview", new ResourceTemplate("tally://user/{address}", { list: undefined }), async (uri, params) => {
  ...
  server.resource("trending-proposals", "tally://trending/proposals", { mimeType: "text/markdown" }, async () => {
  ...
  server.resource("tally-api-schema", "tally://api/schema", { mimeType: "application/json" }, async () => {
    try {
...
function setupAllPrompts(server) {
  Object.entries(governancePrompts).forEach(([name, prompt]) => {
    server.prompt(name, `Governance analysis prompt: ${name.replace(/-/g, " ")}`, prompt.schema, async (args) => {
